...
<xsl:stylesheet	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="timestamp">
   <xsl:value-of select="substring-before(substring-after(//pmd/@timestamp, 'T'), '.')"/> - <xsl:value-of select="substring-before(//pmd/@timestamp, 'T')"/> 
  </xsl:template>

  <xsl:template match="pmd">
    <html>
    <head>
      <title>PMD Report - <xsl:value-of select="$title"/></title>
      <style type="text/css">
        body {
          margin-left: 10;
          margin-right: 10;
      ...
      <body>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
          <tr>
            <td class="text-align:left"><h2>PMD <xsl:value-of select="//pmd/@version"/> Report</h2></td>
            <td class="text-align:center"><h2><xsl:value-of select="$title"/></h2></td>
          </tr><tr>
            <td class="text-align:left"><small><i>Created: <xsl:call-template name="timestamp"/></i></small></td>
          </tr>
        </table>
        <hr size="1"/>
        <!-- Summary part -->
        ...
        <h2><p>Section (ii)</p></h2>
        <xsl:apply-templates select="." mode="filelist"/>
        <hr size="1" width="100%" align="left"/>
        <!-- File part -->
        <a name="top_section3"></a>
        <h2><p>Section (iii)</p></h2>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
          <tr>
            <td><b>End of <a href="http://pmd.sourceforge.net">PMD <xsl:value-of select="//pmd/@version"/></a> report.</b></td>
          </tr>
        </table>
  ...
  <xsl:template match="pmd" mode="summary">
    ...
    <table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
      <tr>
        ...
        <th width="14%">Priority 1</th>
        <th width="14%">Priority 2</th>
        <th width="14%">Priority 3</th>
        <th width="14%">Priority 4</th>
        <th width="14%">Priority 5</th>
      </tr>
      <tr>
  ...
  <xsl:template match="pmd" mode="filelist">
    ...
    <table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
      ...
      <xsl:variable name="highestPriorityColor">
        <xsl:choose><xsl:when test="count(violation[@priority = 1]) &gt; 0">p1</xsl:when>
        <xsl:otherwise>
          <xsl:choose><xsl:when test="count(violation[@priority = 2]) &gt; 0">p2</xsl:when>
          <xsl:otherwise>
            <xsl:choose><xsl:when test="count(violation[@priority = 3]) &gt; 0">p3</xsl:when>
            <xsl:otherwise>
              <xsl:choose><xsl:when test="count(violation[@priority = 4]) &gt; 0">p4</xsl:when>
              <xsl:otherwise>
                <xsl:choose><xsl:when test="count(violation[@priority = 5]) &gt; 0">p5</xsl:when>
                </xsl:choose>
              </xsl:otherwise>
              </xsl:choose>
            </xsl:otherwise>
      ...
      <xsl:variable name="highestPriorityCount">
        <xsl:choose><xsl:when test="count(violation[@priority = 1]) &gt; 0"><xsl:value-of select="count(violation[@priority = 1])"/></xsl:when>
        <xsl:otherwise>
          <xsl:choose><xsl:when test="count(violation[@priority = 2]) &gt; 0"><xsl:value-of select="count(violation[@priority = 2])"/></xsl:when>
          <xsl:otherwise>
            <xsl:choose><xsl:when test="count(violation[@priority = 3]) &gt; 0"><xsl:value-of select="count(violation[@priority = 3])"/></xsl:when>
            <xsl:otherwise>
              <xsl:choose><xsl:when test="count(violation[@priority = 4]) &gt; 0"><xsl:value-of select="count(violation[@priority = 4])"/></xsl:when>
              <xsl:otherwise>
                <xsl:choose><xsl:when test="count(violation[@priority = 5]) &gt; 0"><xsl:value-of select="count(violation[@priority = 5])"/></xsl:when>
                </xsl:choose>
              </xsl:otherwise>
              </xsl:choose>
            </xsl:otherwise>
      ...
      <tr>
        <xsl:call-template name="alternated-row"/>
        <td><div><xsl:attribute name="class"><xsl:value-of select="$highestPriorityColor"/></xsl:attribute>
          <xsl:choose>
            <xsl:when test="$highestPriorityCount = $violationCount">
              <xsl:value-of select="$violationCount"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$highestPriorityCount"/> of <xsl:value-of select="$violationCount"/>
            </xsl:otherwise>
          </xsl:choose>
        </div></td>
        <td><a href="#f-{$myfname}"><xsl:value-of select="$myfname"/></a></td>
      </tr>
      </xsl:for-each>
    </table>
  ...
  <xsl:template match="file">
    ...
    <a name="f-{$myfname}"></a>
    <h3>File: <xsl:value-of select="$myfname"/></h3>
    <table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
      <tr>
        <th width="5%">Priority</th>
        <th width="8%">Line(s)</th>
      ...
      <xsl:for-each select="violation">
        ...
        <tr>
          <xsl:call-template name="alternated-row"/>
          <td><div><xsl:attribute name="class"><xsl:call-template name="priorityDiv"/></xsl:attribute><xsl:value-of disable-output-escaping="yes" select="@priority"/></div></td>
          <td>
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:value-of select="@beginline" /> to <xsl:value-of select="@endline"/>
              </xsl:otherwise>
            </xsl:choose>
          </td>
          <td><xsl:value-of select="@method"/></td>
          <td><xsl:if test="@externalInfoUrl"><a><xsl:attribute name="href"><xsl:value-of select="@externalInfoUrl"/></xsl:attribute><xsl:call-template name="message"/></a></xsl:if><xsl:if test="not(@externalInfoUrl)"><xsl:call-template name="message"/></xsl:if></td>
    ...
    <table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
      <tr>
        <th width="100%">Total number of violations for this class: <xsl:value-of select="count(violation)"/>
            ...
            P5=<xsl:value-of select="count(violation[@priority = 5])"/>)</small>
        </th>
      </tr>
    </table>
