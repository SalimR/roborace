 ...
 */
package net.skhome.twissandra.domain.repository;
...
/**
 * Repository to provide access to user account information.
 *
...
public class CassandraUserRepository implements UserRepository {
	...
	public final UserAccount findUserAccountByUsername(final String username) {
		...
		final List<Column> columns = selector.getColumnsFromRow(COLUMN_FAMILY_USERNAME, username, false, CONSISTENCY_LEVEL_READ);
	...
	public final UserAccount findUserAccountByPrimaryKey(final String uuid) {
		...
		if (!columns.isEmpty()) {
			...
			userAccount.setUsername(Selector.getColumnStringValue(columns, "username"));
			userAccount.setPassword(Selector.getColumnStringValue(columns, "password"));
			return userAccount;
		}
	...
	@Override
	public final boolean createOrUpdateUserAccount(final UserAccount userAccount) {
		...
		mutator.writeColumns(COLUMN_FAMILY_USER, uuid, mutator.newColumnList(mutator.newColumn("username", username), mutator.newColumn("password", password)));
